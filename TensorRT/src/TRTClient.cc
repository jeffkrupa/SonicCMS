...
TRTClient<Client>::~TRTClient() {
    ...
    std::string fileName = std::to_string(getpid()) + "@" + std::string(&hostname[0]) + ".dat";
    
    std::ofstream file("./data/" + fileName);
    std::stringstream msg;
    msg << "Generated by TRTClient" << std::endl;
    if (numOutputTime > 0) {
        msg << "Output time: " << (float)sumOutputTime / numOutputTime << " us" << std::endl;
    }
    else {
        msg << "numOutputTime was zero" << std::endl;
    }
    if (numRemoteTime > 0) {
...
void TRTClient<Client>::getResults(const std::unique_ptr<nic::InferContext::Result>& result) {
    ...
    numOutputTime++;
    sumOutputTime += (unsigned int)std::chrono::duration_cast<std::chrono::microseconds>(t3-t2).count();
}
