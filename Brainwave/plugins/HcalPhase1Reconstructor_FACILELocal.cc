...
class HcalPhase1Reconstructor_FACILETest : public SonicEDProducer<Client>
{
	public:
		...
		explicit HcalPhase1Reconstructor_FACILETest(edm::ParameterSet const& cfg) : 
			...
			fTokDigis(this->template consumes<QIE11DigiCollection>(fDigiName))
		{
			...
			this->setDebugName("HcalPhase1Reconstructor_FACILETest");
		}
		void acquire(edm::Event const& iEvent, edm::EventSetup const& iSetup, Input& iInput) override {

			//std::cout <<"acquire started" << std::endl;
			unsigned int ninput = 15;//client_.ninput();
			unsigned int batchSize = 16000; //client_.batchSize();

                        //float raw_input[batchSize][ninput];
			...
			std::cout <<"processData started" << std::endl;
		        std::vector<float> raw_input = processData<QIE11DataFrame>(*digis, *conditions, ninput, batchSize);

	 		
			std::cout <<"processData done" << std::endl;
			iInput = Input(tensorflow::DT_FLOAT, {batchSize, ninput});
			auto input_map = iInput.template tensor<float, 2>();
			for(unsigned int ib = 0; ib < batchSize; ib++){
				for(unsigned int in = 0 ; in < ninput; in++){
					input_map(ib,in) = raw_input[ib*in + in];
					//iInput.matrix<float>()(ib,in) = raw_input[ib*in + in];
				}
			}
			//std::cout <<"acquire done" << std::endl;
			
		}

		template<class DFrame, class Collection>
		std::vector<float> processData(const Collection& coll,
                                 const HcalDbService& cond,
				 //Input& iInput,
				 //float& iInput, //client_.batchSize()][client_.ninput()],
				 auto ninput, auto batchSize)
		{

			const bool skipDroppedChannels = false;
			std::vector iInput(ninput*batchSize,0.f);
			unsigned int ib = 0;
			for (typename Collection::const_iterator it = coll.begin(); it != coll.end(); it++){


				unsigned int ib = std::distance(coll.begin(),it);
				//std::cout << "started RH " << ib << std::endl;
				...
				for(unsigned int d = 1; d < 30; d++){
					if(std::abs(cell.ieta()) == (float)d) 	{ iInput[ib*ninput + d + 16] = 1.; }
					else 					{ iInput[ib*ninput + d + 16] = 0.; }
				}*/
                                for (int inputTS = 0; inputTS < nCycles; ++inputTS){
                                        auto s(frame[inputTS]);
                                        const uint8_t adc = s.adc();
                                ...
                                iInput[ib*ninput + 0] = cell.ieta();
                                iInput[ib*ninput + 1] = cell.iphi();
				...
				//std::cout << "finished RH " << ib << std::endl;
		...
		void produce(edm::Event& iEvent, edm::EventSetup const& iSetup, Output const& iOutput) override {
			...
			const tensorflow::Tensor& energies = iOutput;
			

  			std::unique_ptr<HBHERecHitCollection> out;
			out = std::make_unique<HBHERecHitCollection>();
  			auto rh_e_list = energies.flat<float>();
			unsigned int ib = 0;
			for(HBHERecHitCollection::const_iterator it = tmp->begin(); it != tmp->end(); it++){

				float rh_e = rh_e_list(ib);
				//std::cout << "RH energy: " << rh_e << std::endl;
				if (rh_e < 0.01) rh_e = 0.0;
		...
		~HcalPhase1Reconstructor_FACILETest() override {}
...
typedef HcalPhase1Reconstructor_FACILETest<TFClientLocal> HcalPhase1Reconstructor_FACILELocal;

DEFINE_FWK_MODULE(HcalPhase1Reconstructor_FACILELocal);
