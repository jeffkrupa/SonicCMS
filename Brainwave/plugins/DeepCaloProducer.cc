...
class DeepCaloProducer : public SonicEDProducer<Client>
{
	public:
		...
		void acquire(edm::Event const& iEvent, edm::EventSetup const& iSetup, Input& iInput) override {
			unsigned int ninput = 2464; //client_.ninput();
			unsigned int batchSize = 5;// client_.batchSize();
			//iInput = Input(ninput*batchSize, 0.f);
			iInput = Input(tensorflow::DT_FLOAT, { 5, 56, 11, 4 });
	...
	private:
		...
		/*void findTopN(const Output& scores) const {
			unsigned int dim = 1; // client_.noutput();
...
typedef DeepCaloProducer<TFClientLocal> DeepCaloProducerLocal;

DEFINE_FWK_MODULE(DeepCaloProducerLocal);
